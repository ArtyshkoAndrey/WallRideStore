...
@section('content')
  <section class="container-fluid p-0 text-white" id="slider">
    <div id="carouselExampleCaptions" class="carousel slide d-none d-md-block" style="height: 500px;" data-ride="carousel">
      ...
      <a class="carousel-control-prev bg-transparent shadow-none" href="#carouselExampleCaptions" role="button" data-slide="prev">
        <span class="carousel-control-prev-icon" aria-hidden="true"></span>
        <span class="sr-only">Previous</span>
      </a>
      <a class="carousel-control-next bg-transparent shadow-none" href="#carouselExampleCaptions" role="button" data-slide="next">
      ...
      <div class="row px-md-5 px-2">
        @if(count($productsTop) > 0)
          @foreach($productsTop as $product)
            <product :slider=true :currency="{{ $currency }}" :item_not_soted="{{ $product }}"></product>
          @endforeach
          @for($i = 0; $i < 5 - $productsTop->count(); $i++)
            <div class="col-lg"></div>
          @endfor
        @else
          <h3 class="text-center mt-5 pb-5">Нет данных товаров</h3>
