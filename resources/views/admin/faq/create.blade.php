  ...
  <div class="container-fluid mt-20 mb-20">
    <div class="row row-eq-spacing">
      <div class="col-12">
        <nav aria-label="Breadcrumb navigation example">
          <ul class="breadcrumb">
            ...
            <li class="breadcrumb-item active">Создание FAQ</li>
          </ul>
        </nav>
      </div>
      <div class="col-12">
      ...
      <div class="col-12 p-0">
        <form action="{{ route('admin.faq.store') }}" method="POST" class="w-full">
          @csrf
...
@section('script')
  ...
  <script>
    ...
    $(document).ready(() => {
      ...
      let mockFile
      @if (old('image'))
        mockFile = { name: '{{ old('image') }}', size: {{ File::size(public_path(\App\Models\Faqs::PHOTO_PATH . old('image') . '.jpg')) }} };
        uploader.emit("addedfile", mockFile);
        uploader.emit("thumbnail", mockFile, '{{ asset(\App\Models\Faqs::PHOTO_PATH . old('image') . '.jpg') }}');
        uploader.emit("complete", mockFile);
        uploader.files.push(mockFile)
        fileList.push({"serverFileName": '{{ old('image') }}', "fileName":'{{ old('image') }}', "fileId": 1});
      @endif
    })
  </script>
@endsection
