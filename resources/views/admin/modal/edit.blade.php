...
@section('title', 'Редактирование модального окна')
  ...
  <div class="container-fluid mt-20 mb-20">
    <div class="row row-eq-spacing">
      <div class="col-12">
        <nav aria-label="Breadcrumb navigation example">
          <ul class="breadcrumb">
            ...
            <li class="breadcrumb-item active">Редактирование модального окна</li>
          </ul>
        </nav>
      </div>
      <div class="col-12">
        <h3>Редактирование модального окна</h3>
      </div>
      @if ($errors->any())
        <div class="col-12">
      ...
      <div class="col-12 p-0">
        <form action="{{ route('admin.modal.update', $modal->id) }}" method="POST" class="w-full">
          @csrf
          @method('PUT')
                    ...
                    <div class="wrapper-tabs">
                      ...
                      <div class="tabs-content active" id="russian">
                        ...
                        <div class="form-group">
                          ...
                          <input type="text" class="form-control" name="ru[title]" id="title" placeholder="Название" value="{{ old('ru.title', $modal->{'title:ru'}) }}" required>
                        </div>

                        <div class="form-group">
                          <label for="ru_description">Описание</label>
                          <textarea class="form-control" name="ru[description]" id="ru_description" cols="30" rows="20">{{ old('ru.description', $modal->{'description:ru'}) }}</textarea>
                        </div>

                        <div class="form-group">
                          <label for="ru_text_to_link">Текст на кнопке</label>
                          <input type="text" class="form-control" name="ru[text_to_link]" id="ru_text_to_link" placeholder="Текст на кнопке" value="{{ old('ru.text_to_link', $modal->{'text_to_link:ru'}) }}">
                        </div>
                      ...
                      <div class="tabs-content" id="english">
                        ...
                        <div class="form-group">
                          <label for="title" class="required">Заголовок</label>
                          <input type="text" class="form-control" name="en[title]" id="title" placeholder="Название" value="{{ old('en.title', $modal->{'title:en'}) }}" required>
                        </div>

                        <div class="form-group">
                          <label for="en_description">Описание</label>
                          <textarea class="form-control" name="en[description]" id="en_description" cols="30" rows="20">{!! old('en.description', $modal->{'description:en'}) !!}</textarea>
                        </div>

                        <div class="form-group">
                          <label for="en_text_to_link">Текст на кнопке</label>
                          <input type="text" class="form-control" name="en[text_to_link]" id="en_text_to_link" placeholder="Текст на кнопке" value="{{ old('en.text_to_link', $modal->{'text_to_link:en'}) }}">
                        </div>
                ...
                <div class="col-12">
                  <div class="card bg-dark-dm m-0">
                    ...
                    <div class="form-group">
                      ...
                      <input type="text" class="form-control" name="link" id="link" placeholder="Ссылка на кнопке" value="{{ old('link', $modal->link) }}">
                    </div>
                ...
                <div class="row">
                  <div class="col-12">
                    <div class="custom-switch d-inline-block">
                      ...
                      <input type="checkbox" name="on_auth" id="on_auth" value="1" {{ old('on_auth', $modal->on_auth) ? 'checked' : null }}>
                  ...
                  <div class="col-12 mt-10">
                    <div class="custom-switch d-inline-block">
                      ...
                      <input type="checkbox" name="status" id="status" value="1" {{ old('status', $modal->status) ? 'checked' : null }}>
                  ...
                  <div class="col-12" id="checker_images">
                    ...
                    <ul class="row">
                      <li class="col-md-12 col-lg-6 col-xl-4 col-12">
                        <input type="radio" name="type" value="1" id="cb1" {{ old('type', $modal->type) === 1 || old('type', $modal->type) === null ? 'checked' : '' }}/>
                        <label for="cb1"><img src="{{ asset('images/modals/modal_1.png') }}" alt=""/></label>
                      </li>
                      <li class="col-md-12 col-lg-6 col-xl-4 col-12">
                        <input type="radio" name="type" value="2" id="cb2" {{ old('type', $modal->type) === 2 ? 'checked' : '' }} />
                        <label for="cb2"><img src="{{ asset('images/modals/modal_2.png') }}" alt=""/></label>
                      </li>
                      <li class="col-md-12 col-lg-6 col-xl-4 col-12">
                        <input type="radio" name="type" value="3" id="cb3" {{ old('type', $modal->type) === 3 ? 'checked' : '' }} />
                        <label for="cb3"><img src="{{ asset('images/modals/modal_3.png') }}" alt=""/></label>
                      </li>
                      <li class="col-md-12 col-lg-6 col-xl-4 col-12">
                        <input type="radio" name="type" value="4" id="cb4" {{ old('type', $modal->type) === 4 ? 'checked' : '' }} />
          ...
          @if(old('image', $modal->image))
            <input type="hidden" name="image" id="{{ old('image', $modal->image) }}" value="{{ old('image', $modal->image) }}">
          @endif
        </form>
      </div>
    </div>
...
@section('script')
  ...
  <script>
    ...
    $(document).ready(function() {
      let mockFile
      @if (old('photo', $modal->image))
        mockFile = { name: '{{ old('photo', $modal->image) }}', size: {{ File::size(public_path(\App\Models\Modal::PHOTO_PATH . old('photo', $modal->image) . '.jpg')) }} };
      uploader.emit("addedfile", mockFile);
      uploader.emit("thumbnail", mockFile, '{{ asset(\App\Models\Modal::PHOTO_PATH . old('photo', $modal->image) . '.jpg') }}');
      uploader.emit("complete", mockFile);
      uploader.files.push(mockFile)
      fileList.push({"serverFileName": '{{ old('photo', $modal->image) }}', "fileName":'{{ old('photo', $modal->image) }}', "fileId": 1});
      @endif
    });
  </script>
@endsection
