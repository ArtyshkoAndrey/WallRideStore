...
@section('content')
  <div class="container-fluid pt-5 px-4">
    <div class="row">
      ...
      <div class="col-12 col-md-auto">
        <a href="{{ route('admin.header.create') }}" class="btn btn-dark rounded-0 border-0">Добавить новый</a>
      </div>
    </div>
          ...
          <table class="table text-nowrap">
            ...
            @forelse($hs as $h)
              <tr class="align-items-center">
                <td style="vertical-align: middle; width: 200px"><img src="{{ asset('storage/header/' . $h->photo) }}" class="img-fluid" alt="{{ $h->photo }}"></td>
                <td style="vertical-align: middle;"><a href="{{ route('admin.header.edit', $h->id) }}" class="text-red">{{$h->h1}}</a></td>
                <td style="vertical-align: middle;">{{$h->h2}}</td>
                <td style="vertical-align: middle;">{{$h->url}}</td>
                <td style="vertical-align: middle;">
                  <form action="{{ route('admin.header.destroy', $h->id) }}" method="post">
                    @csrf
                    @method('delete')
