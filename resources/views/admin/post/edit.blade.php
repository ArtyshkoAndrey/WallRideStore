  ...
  <div class="container-fluid mt-20 mb-20">
    <div class="row row-eq-spacing">
        ...
        <nav aria-label="Breadcrumb navigation example">
          <ul class="breadcrumb">
            <li class="breadcrumb-item">
              <a href="{{ route('admin.post.index') }}">Новости</a>
            </li>
      ...
      <div class="col-12 p-0">
        <form action="{{ route('admin.post.update', $post) }}" method="POST" class="w-full">
          @csrf
          @method('PUT')
            ...
            <div class="col-12 col-lg mt-10">
              <div class="card bg-dark-dm">
                <div class="row row-eq-spacing-sm m-0 p-0">
                    ...
                    <div class="wrapper-tabs">
                      ...
                      <div class="tabs-content active" id="russian">
                        ...
                        <div class="form-group">
                          ...
                          <input type="text" class="form-control" name="ru[title]" id="ru_title" placeholder="Название" value="{{ old('ru.title', $post->{'title:ru'}) }}" required>
                        </div>
                      ...
                      <div class="tabs-content" id="english">
                        ...
                        <div class="form-group">
                          ...
                          <input type="text" class="form-control" name="en[title]" id="en_title" placeholder="Название" value="{{ old('en.title', $post->{'title:en'}) }}" required>
                        </div>
                  ...
                  @if($photo = old('photo', $post->photo))
                    @if(File::exists(public_path(\App\Models\Post::PHOTO_PATH . $photo . '.jpg')))
  ...
  <script>
    ...
    const uploader = new Dropzone('#upload-widget', {
      init: function() {
        ...
        this.on("removedfile", function(file) {
          ...
          if (rmvFile){
            console.log(rmvFile)
            axios.post("{{ route('admin.post.photo.delete') }}", {
              name: rmvFile
            })
              .then(response => {
      ...
      maxFiles: 1,
      acceptedFiles: 'image/*',
      url: "{{ route('admin.post.photo.store') }}",
    ...
    $(document).ready(() => {
      tinymce.init({
        ...
        images_upload_url: '{{ route('admin.post.content-image') }}',
      ...
      @if ($photo = old('photo', $post->photo))
        @if(File::exists(public_path(\App\Models\Post::PHOTO_PATH . $photo . '.jpg')))
          mockFile = { name: '{{ $photo }}', size: {{ File::size(public_path(\App\Models\Post::PHOTO_PATH . $photo . '.jpg')) }} };
          uploader.emit("addedfile", mockFile);
          uploader.emit("thumbnail", mockFile, '{{ asset(\App\Models\Post::PHOTO_PATH . $photo . '.jpg') }}');
          uploader.emit("complete", mockFile);
          uploader.files.push(mockFile)
          fileList.push({"serverFileName": '{{ $photo }}', "fileName":'{{ $photo }}', "fileId": 1});
        @endif
      @endif
    })
  </script>
