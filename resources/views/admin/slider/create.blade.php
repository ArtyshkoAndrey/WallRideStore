  ...
  <div class="container-fluid mt-20 mb-20">
    <div class="row row-eq-spacing">
      ...
      <div class="col-12 p-0">
        <form action="{{ route('admin.slider.store') }}" method="POST" class="w-full">
          @csrf
                ...
                <div class="row row-eq-spacing-sm m-0 p-0">
                    ...
                    <div class="wrapper-tabs">
                      ...
                      <div class="tabs-content active" id="russian">
                        ...
                        <div class="form-group">
                          ...
                          <input type="text" class="form-control" name="ru[h1]" id="ru_h1" placeholder="Заголовок" value="{{ old('ru.h1') }}" required>
                        </div>

                        <div class="form-group">
                          <label for="ru_h2" class="required">Подзаголовок</label>
                          <input type="text" class="form-control" name="ru[h2]" id="ru_h2" placeholder="Подзаголовок" value="{{ old('ru.h2') }}" required>
                        </div>

                        <div class="form-group">
                          <label for="ru_btn_text" class="required">Текст на кнопке</label>
                          <input type="text" class="form-control" name="ru[btn_text]" id="ru_btn_text" placeholder="Текст на кнопке" value="{{ old('ru.btn_text') }}" required>
                        </div>
                      ...
                      <div class="tabs-content" id="english">
                        ...
                        <div class="form-group">
                          ...
                          <input type="text" class="form-control" name="en[h1]" id="en_h1" placeholder="Заголовок" value="{{ old('en.h1') }}" required>
                        </div>

                        <div class="form-group">
                          <label for="en_h2" class="required">Подзаголовок</label>
                          <input type="text" class="form-control" name="en[h2]" id="en_h2" placeholder="Подзаголовок" value="{{ old('en.h2') }}" required>
                        </div>

                        <div class="form-group">
                          <label for="en_btn_text" class="required">Текст на кнопке</label>
                          <input type="text" class="form-control" name="en[btn_text]" id="en_btn_text" placeholder="Текст на кнопке" value="{{ old('en.btn_text') }}" required>
                        </div>
                  ...
                  <div class="col-12">
                    ...
                    <div class="form-group">
                      ...
                      <input type="text" class="form-control" name="url" id="url" placeholder="Ссылка на кнопке" value="{{ old('url') }}" required>
                    </div>
                  </div>
                  ...
                  @if(old('photo'))
                    <input type="hidden" name="photo" id="{{ old('photo') }}" value="{{ old('photo') }}">
                  @endif
                  ...
                  @if(old('mobile_photo'))
                    <input type="hidden" name="mobile_photo" id="{{ old('mobile_photo') }}" value="{{ old('mobile_photo') }}">
                  @endif
...
@section('script')
  ...
  <script>
    ...
    $(document).ready(() => {
      ...
      let mockFile
      @if (old('photo'))
        mockFile = { name: '{{ old('photo') }}', size: {{ File::size(public_path(\App\Models\Slider::PHOTO_PATH . old('photo') . '.jpg')) }} };
        uploader.emit("addedfile", mockFile);
        uploader.emit("thumbnail", mockFile, '{{ asset(\App\Models\Slider::PHOTO_PATH . old('photo') . '.jpg') }}');
      ...
      @if (old('mobile_photo'))
        mockFile = { name: '{{ old('mobile_photo') }}', size: {{ File::size(public_path(\App\Models\Slider::PHOTO_PATH_MOBILE . old('mobile_photo') . '.jpg')) }} };
        uploaderMobile.emit("addedfile", mockFile);
        uploaderMobile.emit("thumbnail", mockFile, '{{ asset(\App\Models\Slider::PHOTO_PATH_MOBILE . old('mobile_photo') . '.jpg') }}');
        uploaderMobile.emit("complete", mockFile);
        uploaderMobile.files.push(mockFile)
        fileListMobile.push({"serverFileName": '{{ old('photo') }}', "fileName":'{{ old('mobile_photo') }}', "fileId": 1});
      @endif
    })
  </script>
@endsection
