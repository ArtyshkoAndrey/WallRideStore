...
@section('content')
  <section class="container mt-5 pt-5 mb-5" id="cart">
    <create-order :amount="{{ $amount }}" :cart_items="{{ json_encode($cartItems) }}" inline-template>
      <div class="row">
        <div class="col-12">
          <div class="row">
              ...
              @forelse($cartItems as $item)
                <div class="row mt-3 justify-content-center align-items-center">
                  ...
                  <div class="col-4 col-md text-center font-weight-bold">
                    {{ round($item->productSku->price * $currency->ratio, 0) }} {{ $currency->symbol }} X {{ $item->amount }}
                  </div>
                </div>
              @empty
              ...
              <div class="row p-2">
                ...
                <div class="col-md-6 mt-3 mt-md-0">
                  ...
                  <div class="btn-group btn-group-toggle" data-toggle="buttons" v-if="!order.pickup">
                    <label class="btn btn-white border-0 rounded-0 p-3" :disabled="order.pickup" @click="() => {!order.pickup ? order.express_company = 'ase' : null}">
                      <input type="radio" value="ase" name="express_company" id="option3" autocomplete="off" :checked="order.express_company === 'ase'"> Asia sky Expreess
                    </label>
        ...
        <div class="col-12 mt-3">
          <div :class="(step === 1 ? 'justify-content-end' : 'justify-content-between') + ' row'">
            <div class="col-sm-auto col-12 ml-2 ml-sm-0" v-if="step === 2">
              <h4>Общая сумма {{ round($priceAmount * $currency->ratio, 0) }} {{ $currency->symbol }}</h4>
            </div>
