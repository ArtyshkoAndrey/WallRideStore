<?php
return [
  "product" => [
    "product_show" => "Данного товара не существует",
    "product_catalog_skuses" => "Выбраны неверные размеры",
    "product_catalog_brands" => "Выбраны неверные бренды",
    "product_catalog_categories" => "Выбраны неверные категории",
    "product_search" => "Поле поиск не может быть пустым"
  ],
  "auth" => [
    "admin" => "Доступ открыть только администраторам"
  ],
  "delivery" => [
    "error" => "Ошибка расчётов доставка",
    "not_price" => "Возможно в ваш город нет доставки"
  ]

];
