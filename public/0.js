...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  methods: {
    ...
    favored: function favored() {
      var _this3 = this;

      // нициируйте запрос post ajax. URL запроса генерируется функцией backend route ().
      axios.post('/products/' + this.product.id + '/favorite').then(function () {
        swal('Операция прошла успешно', '', 'success').then(function () {
          // здесь добавлен метод then ()
          _this3.favoredData = true;
        });
      }, function (error) {
        // Этот обратный вызов будет выполнен в случае сбоя запроса
    ...
    disFavored: function disFavored() {
      var _this4 = this;

      axios.delete('/products/' + this.product.id + '/favorite').then(function () {
        swal('Операция прошла успешно', '', 'success').then(function () {
          _this4.favoredData = false;
        });
      });
    }
  },
